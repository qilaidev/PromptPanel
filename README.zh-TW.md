...
- 📌 **置頂 + frecency 排序**：必須常駐的詞條置頂；其餘按 `使用次數 × 2^(-閒置天數 / 90)` 計分，高頻詞條自動上浮，久未使用的自然下沉
- 🌗 **淺色 / 深色 / 跟隨系統**
- 🪶 **選單列常駐**：不打擾，要用就來
...
2. PromptPanel 把 `NSPanel` 彈到目前視窗上方，自動聚焦搜尋框，按「置頂 → frecency 得分 → 最近使用 → 使用次數」展示目前專案 + `通用專案` 詞條
...
| *(空)* | 瀏覽「目前專案 + 通用專案」，依 置頂 → frecency → 最近使用 → 使用次數 排列 |
...
優先順序和取捨規則見 [docs/路线图与贡献指南.md](docs/路线图与贡献指南.md)，已發布內容看 [CHANGELOG.md](CHANGELOG.md)，公開規劃看 [issues](https://github.com/qilaidev/PromptPanel/issues)。
...
提 issue：<https://github.com/qilaidev/PromptPanel/issues>，請用範本，能減少很多來回。
