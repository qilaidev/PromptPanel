...
- 该目录本身**只读**，内容由设计源（`index.html` + `components/*.jsx`）构成，不在运行期被引用，只在代码评审时作为对齐凭据
- 仓库不再存放截图产物；需要渲染图时本地跑 `./scripts/capture-ui-qa.sh`，输出已 git-ignore（见 `.github/CONTRIBUTING.md`）
...
| `openspec/` | SDD 规格与变更记录；影响架构、运行、发布、运维或文档基线的开发任务先在这里记录上下文 |
...
- 清理 30 天前执行日志
- 自动生成启动备份
- 启动备份只保留最近 7 份；导入前的安全快照保留最近 5 份；用户手动创建的备份不会被自动清理
- 手动备份长期保留
- 暴露运行健康快照给主界面
...
项目开发默认采用 OpenSpec 作为规格驱动基线：

- 当前事实放在 `openspec/specs/`
- 待实施或正在实施的变更放在 `openspec/changes/<change-id>/`
- 影响文档体系的当前规格是 `openspec/specs/project-documentation/spec.md`
- 本次完整文档体系整理记录在 `openspec/changes/complete-project-docs/`

OpenSpec 不替代 `docs/`。它负责记录“为什么改、规格如何变化、任务如何验收”；`docs/` 负责成为维护者日常可读、可执行、可排障的事实手册。
...
- 影响架构、数据、发布、运维或文档体系的变更先补 OpenSpec change，再进入实现；紧急止血后也要补回规格和变更记录。
